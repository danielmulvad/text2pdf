  ...
  while (!atEOF) {
    ...
    while (column++ <= columns) {
      ...
      while (lineNo++ < lines && !atEOF && !atFF) {
        writestr("(");
        charNo = 0;
        while (charNo++ < cols && (ch = getc(infile)) != EOF &&
               !(ch == FF && doFFs) && ch != '\n') {
          if (ch >= 32 && ch <= 127) {
...
void ShowHelp() {
  printf("\n%s [options] [filename]\n\n", progname);
  printf(
      "  %s makes a 7-bit clean PDF file (version 1.1) from any input file.\n",
      progname);
  printf(
      "  It reads from standard input or a named file, and writes the PDF "
      "file\n");
  ...
  printf(
      "  -l<lines>\tlines per page (default 60, determined "
      "automatically\n\t\tif unspecified)\n");
  ...
  printf(
      "  In landscape mode, page width and height are simply swapped over "
